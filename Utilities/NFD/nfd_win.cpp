...
{
    return NFD_OKAY; // function is deprecated because we do not have the SMC create function apparently
    ...
    IShellItem *folder;
    // this function apparently does not exist, probably not in our version of c++, so get screwed
    HRESULT result = S_OK; // SHCreateItemFromParsingName(defaultPathW, NULL, IID_PPV_ARGS(&folder));
