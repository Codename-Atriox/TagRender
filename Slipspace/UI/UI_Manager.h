...
public:
	...
	void render_module_window(ModuleManager* Modules) {
		...
		for (int i = 0; i < Modules->open_modules; i++) {
			...
			if (ImGui::CollapsingHeader(menu_active_module->filepath.c_str())) {
				ImGui::DragInt("Selected File", &module_file_index, 0.1f, 0, menu_active_module->file_count);
				...
				for (uint32_t tag_ind = 0; tag_ind < max_files_onscreen_count; tag_ind++) {
					...
					uint32_t tag_moduleindex = tag_ind + module_file_index;
	...
	// //////////// //
	CTList<Tag> OpenTags;
	void render_bitmap_window(ModuleManager* Modules) {
